...
def uploadManu():
    s3 = boto3.client('s3',
                      aws_access_key_id=aws_access,
                      aws_secret_access_key=aws_secret)
    new_df=pd.read_csv('https://markjbs.s3.us-west-2.amazonaws.com/Brand-supplier.csv',dtype='object')
    new_df_data=new_df.values.tolist()
    brand_new_lower_set = pd.read_csv('https://markjbs.s3.us-west-2.amazonaws.com/Brand-supplier1.csv',dtype='object').dropna(how='all').reset_index(drop=True).values.tolist()
    for i in brand_new_lower_set:
        if(i not in new_df_data):
            new_df_data.append(i)
    # brand_lower_set=brand_lower_set.union(brand_new_lower_set)
...
def testingg(column):
    ...
    df=pd.read_csv('https://markjbs.s3.us-west-2.amazonaws.com/input.csv',dtype='object')
